...
DST_game_path="$HOME/Steam/steamapps/common/DSTServer"
...
checkping(){
    list="steamcommunity.com kleientertainment.com hk.wqlin.com"
    for i in $list; do
        pingc "$i"
    done
}
...
checklib(){
    if [ $bit -eq 64 ]; then
        lib="lib32gcc1 lib32stdc++6 libcurl4-gnutls-dev:i386"
    else
        lib="libcurl4-gnutls-dev libgcc1 libstdc++6"
    fi
    list="$lib screen grep lua5.2 diffutils htop"
    for i in $list; do
        dpkg -s $i &> /dev/null
        if [ $? -gt 0 ]; then
